$(function () {
    $("#jqGrid").jqGrid({
        url: '../sys/sysftpconfig/list',     // 请求后台json数据的url
        ...
        colModel: [
            ...
            {label: 'FTP地址', name: 'host', index: 'host', width: 80 },
            {label: '端口', name: 'port', index: 'port', width: 80 },
            {label: '根路径', name: 'rootpath', index: 'rootpath', width: 120 },
            {label: 'URL地址', name: 'origin', index: 'origin', width: 120 },
        ],
    ...
    methods: {
        query: function () {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {
                    id: vm.sysFtpConfig.id,
                    remark: vm.sysFtpConfig.remark
                },
                page: 1
            }).trigger("reloadGrid");
        },
        ...
        saveOrUpdate: function (event) {
            var url = vm.sysFtpConfig.id == null ? "../sys/sysftpconfig/save" : "../sys/sysftpconfig/update";
            $.ajax({
                type: "POST",
                url: url,
                data: JSON.stringify(vm.sysFtpConfig),
                success: function(r){
                    if(r.code === 0){
        ...
        getInfo: function(id){
            $.get("../sys/sysftpconfig/info/"+id, function(r){
                vm.sysFtpConfig = r.sysFtpConfig;
            });
        },
