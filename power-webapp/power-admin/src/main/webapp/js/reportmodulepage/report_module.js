...
function forSelectOption(data_url_id, executeCode) {
    $(data_url_id).empty();
    $(data_url_id).append("<option value=''>-- 空 --</option>");
    var selectOption;
    /*
     // sql语句
...
$(".saveOrUpdate").click(function () {
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            if (getStringValue(returnJsonObj.reportModulePage.id) != "") {
                module_id = returnJsonObj.reportModulePage.id;
                reportModulePage.code = returnJsonObj.reportModulePage.code;
            }
            if (returnJsonObj === 0) {
...
function selfModuleJson() {
    ...
    $plugjsonstr.each(function () {
        ...
        if (getStringValue($this.val()) != "") {
            jsonObj.parts.push(JSON.parse($this.val()));
        }
    });
    return jsonObj;
}
